...
int Fun4All_G4_sPHENIX(
    const int nEvents = 1,
    const int inputfilelistidx = 0,
    const string &outputFile = "MVTXRecoClusters_test",
    const int skip = 0)
{
...
//     }
  ...
  if (Enable::TRACKING_QA && Enable::CEMC_QA && Enable::HCALIN_QA && Enable::HCALOUT_QA) QA_G4CaloTracking();
    ...
    bool isData = false;
    // test *mytest = new test(outputFile, isData, nEvents * inputfilelistidx);
    dNdEtaAnalyzer *myAnalyzer = new dNdEtaAnalyzer(outputFile, isData, nEvents * inputfilelistidx);
