...
// #include <mvtxalignment/writeHits.h>
#include <test/test.h>

R__LOAD_LIBRARY(libfun4all.so)
// R__LOAD_LIBRARY(libmvtxhitfile.so)
R__LOAD_LIBRARY(libtest.so)
...
int Fun4All_G4_sPHENIX_SimplePion(
    const int nEvents = 1,
    const int nPart = 2000,
    const float vtxZ_mean = 0.,
    const float vtxZ_width = 0.)
{
    ...
    if (Input::SIMPLE)
    {
        INPUTGENERATOR::SimpleEventGenerator[0]->add_particles("pi-", nPart);
        ...
        else
        {
            INPUTGENERATOR::SimpleEventGenerator[0]->set_vertex_distribution_function(PHG4SimpleEventGenerator::Gaus,
                                                                                      ...
                                                                                      PHG4SimpleEventGenerator::Gaus);
            INPUTGENERATOR::SimpleEventGenerator[0]->set_vertex_distribution_mean(0., 0., vtxZ_mean);
            INPUTGENERATOR::SimpleEventGenerator[0]->set_vertex_distribution_width(0., 0., vtxZ_width);
        }
    ...
    */
    ...
    // writeHits *myMVTXHits = new writeHits(Form("myMVTXHits_SIMPLE%dPions_VtxZmean%dcm_VtxZwidth%dcm",nPart, (int) vtxZ_mean, (int) vtxZ_width));
    ...
    bool isData = false;
    test *mytest = new test(Form("MVTXRecoClusters_SIMPLE%dPions_VtxZmean%dcm_VtxZwidth%dcm",nPart, (int) vtxZ_mean, (int) vtxZ_width), isData, nEvents);
    mytest->GetTruthPV(false);
    mytest->GetG4Particle(true);
    mytest->GetRecoCluster(true);
    mytest->GetMVTXhits(false);
